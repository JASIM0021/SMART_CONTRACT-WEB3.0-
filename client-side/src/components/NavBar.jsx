    ...
    <ul className='text-white md:flex hidden list-none flex-row justify-between items-center flex-initial'>
      {
        ["Market","Exchange","Tutorials","Wallets"].map((item,index)=>(
          <NavbarItems  title={item} key={index} classProps=''/>
        ))
      }
      <li className='bg-blue-500 py-2 px-7 mx-4 rounded-full cursor-pointer hover:bg-blue-400'>Login</li>
    </ul> 
      ...
      }
      {
        toggleMenu && (
          ...
          >
            ...
            {
        ["Market","Exchange","Tutorials","Wallets"].map((item,index)=>(
          <NavbarItems  title={item} key={index} classProps='my-2 text-lg'/>
        ))
      }
      <li className='bg-blue-500 py-2 px-7 mx-4 rounded-full cursor-pointer hover:bg-blue-400'>Login</li>
